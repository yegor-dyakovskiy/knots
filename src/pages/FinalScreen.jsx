...
export default function FinalScreen() {
  ...
  const copyResults = async () => {
    ...
    try {
      ...
      // Через 2 секунды вернуть текст кнопки обратно
  ...
  const quotes = [
    ...
    { text: "Я думаю у каждого альпиниста есть много своих поводов взойти на Маттерхорн. Но основная причина у всех одна: взойти на Маттерхорн",  author: "Гастон Реббюфа"},
    { text: "Мои партнеры должны были быть сильными, скромными, быстрыми и всегда оптимистичными.",  author: "Симоне Моро"},
    { text: "Альпинизм – сложный и опасный многогранный вид деятельности человека.",  author: "Виталий Абалаков"},
    ...
    { text: "Каждый человек нуждается в чем-то исключительном.", author: "Райнхольд Месснер" },
    ...
    { text: "Вершину нельзя покорить. Ты стоишь на ней считаные минуты.", author: "Арлен Блум" },
    { text: "Гора искренняя. Орудия для её покорения находятся внутри тебя.", author: "Вальтер Бонатти" },
  ];
  ...
  return (
    <PageWrapper className={`${isExiting ? "fade-out" : ""} no-overflow`}>
      ...
      <div className="final-input-box">
        ...
        <button onClick={sendToCoach} className="start-button">
          Отправить
        </button>
      </div>
  <div className="final-divider"></div>
      <div className="center-div" style={{ marginTop: "15px" }}>
        <button onClick={copyResults} className="start-button">
